...
class InferenceEngine:
        ...
        for name, path in [("phase1b", adapter_1b), ("phase2", adapter_2), ("onco", adapter_onco)]:
            ...
            if not os.path.exists(config_file):
                raise FileNotFoundError(
                    ...
                    f"Please ensure the adapter directory contains all required files. "
    ...
    async def infer_phase1b(self, case_text: str, patient_history: list | None = None) -> tuple[str, float, str, bool, str | None]:
        ...
        prompt = case_text
        # Add patient history context for phase1b
        if patient_history and len(patient_history) > 0:
            history_text = "\n\nPatient History (Previous Check-ins):\n"
            for prev in patient_history[-5:]:
                history_text += f"\nDay {prev.get('pod', 'N/A')}: Pain {prev.get('pain_score', 'N/A')}/10, "
                history_text += f"Temp {prev.get('temperature', 'N/A')}°C, Risk: {prev.get('risk_level', 'N/A')}"
            prompt = history_text + "\n\n" + prompt
    ...
    async def infer_phase2(
        ...
        if patient_history and len(patient_history) > 0:
            history_text = "\n\nPatient History (Previous Check-ins):\n"
            for idx, prev in enumerate(patient_history[-5:], 1):  # Last 5 check-ins
                history_text += f"\nDay {prev.get('pod', 'N/A')}: Pain {prev.get('pain_score', 'N/A')}/10, "
                history_text += f"Temp {prev.get('temperature', 'N/A')}°C, "
                history_text += f"Mobility: {prev.get('mobility', 'N/A')}, "
                if prev.get('wound_concerns') and prev.get('wound_concerns') != 'none':
                    history_text += f"Wound: {prev.get('wound_concerns')}, "
                if prev.get('nausea_vomiting'):
                    history_text += "Nausea/vomiting present, "
                history_text += f"Risk: {prev.get('risk_level', 'N/A')}"
            prompt = history_text + "\n\n" + prompt
    ...
    async def infer_onco(self, case_text: str, patient_history: list | None = None) -> tuple[str, float, str, bool, str | None]:
        ...
        prompt = case_text
        # Add patient history context for onco
        if patient_history and len(patient_history) > 0:
            history_text = "\n\nPatient History (Previous Check-ins):\n"
            for prev in patient_history[-5:]:
                history_text += f"\nDay {prev.get('pod', 'N/A')}: Pain {prev.get('pain_score', 'N/A')}/10, "
