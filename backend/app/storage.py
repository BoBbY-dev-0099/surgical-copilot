...
def _init_default_patients(conn: sqlite3.Connection) -> None:
    ...
    for patient_data in default_patients:
        try:
            ...
            # Map values explicitly, handle unexpected lengths gracefully
            # patient_data should have 13 elements, +2 for now/now = 15
            vals = list(patient_data) + [now, now]
            
            # Truncate or pad if necessary to match columns
            ...
            query = f"INSERT INTO patients ({','.join(columns)}) VALUES ({qmarks})"
            ...
            logger.error(f"Failed to seed patient {patient_data[0] if patient_data else 'unknown'}: {e}")
